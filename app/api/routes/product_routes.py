...
async def create_product(name: str = Form(...),
    ...
    return ProductService.create_product(name, slug, description, price, category_id, image, session)

@product_router.put("/{slug}", response_model=ProductResponseSchema)
async def update_product(slug: str, 
                         body: UpdateProductSchema,
                         session: Session = Depends(get_session)):
    return ProductService.update_product(slug, body, session)
...
async def deactivate_product(slug: str, session: Session = Depends(get_session)):
    ProductService.deactivate_product(slug, session)

@product_router.patch("/{slug}/activate")
async def deactivate_product(slug: str, session: Session = Depends(get_session)):
    ProductService.activate_product(slug, session)